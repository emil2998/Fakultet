...
namespace VanjskeBiblioteke
{
    public static class ValidacijaUnosa
    {
        ...
        public static string ProvjeriBrojRedova(string broj_redova)
        {
            string povratnaPoruka = "";
        
            if (broj_redova.All(Char.IsDigit) == false)
            {
                povratnaPoruka += "Broj redova može sadržavati samo brojeve!\n";
            }
            if (broj_redova == "")
            {
                povratnaPoruka += "Broj redova mora biti unesen!\n";
            }
            return povratnaPoruka;
        }

        public static string ProvjeriBrojStupaca(string broj_stupaca)
        {
            string povratnaPoruka = "";

            if (broj_stupaca.All(Char.IsDigit) == false)
            {
                povratnaPoruka += "Broj stupaca može sadržavati samo brojeve!\n";
            }
            if (broj_stupaca == "")
            {
                povratnaPoruka += "Broj stupaca mora biti unesen!\n";
            }
            return povratnaPoruka;
        }

        public static string ProvjeriVrijeme(string vrijeme)
        {
            string povratnaPoruka = "";
           
            if (vrijeme.All(c => Char.IsDigit(c) || c == ':') ==false)
            {
                povratnaPoruka += "Vrijeme mora biti u formatu npr. 20:00 !\n";
            }
            if (vrijeme.Contains(":")==false)
            {
                povratnaPoruka += "Vrijeme mora biti u formatu npr. 20:00 !\n";
            }
            if (vrijeme.Length>5 || vrijeme.Length <5)
            {
                povratnaPoruka += "Vrijeme mora biti u formatu npr. 20:00 !\n";
            }
            ...
            if (vrijeme == "")
            {
                povratnaPoruka += "Vrijeme mora biti uneseno!\n";
            }
            return povratnaPoruka;
        }

        public static string ProvjeriIznos(string iznos)
        {
            string povratnaPoruka = "";

            if (iznos.All(Char.IsDigit) == false)
            {
                povratnaPoruka += "Iznos može sadržavati samo brojeve!\n";
            }
            if (iznos == "")
            {
                povratnaPoruka += "Iznos mora biti unesen!\n";
            }
            return povratnaPoruka;
        }
